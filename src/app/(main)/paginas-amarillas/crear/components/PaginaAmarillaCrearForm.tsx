...
const paginaAmarillaSchema = z.object({
  ...
  telefonoContacto: z.string().regex(/^[+]?\d[0-9\s\-()]{6,19}$/u, 'Número de teléfono no válido').optional().or(z.literal('')).nullable(),
  emailContacto: z.string().email('Email no válido').optional().or(z.literal('')).nullable(),
  enlaceWeb: z.string().url('URL de sitio web no válida').optional().or(z.literal('')).nullable(),
  enlaceInstagram: z.string().url('URL de Instagram no válida').optional().or(z.literal('')).nullable(),
  enlaceFacebook: z.string().url('URL de Facebook no válida').optional().or(z.literal('')).nullable(),
...
interface UserProfileExtra { nombre?: string; provincia?: string | ProvinciaEnLocalidad; localidad?: LocalidadPerfil; rubro?: RubroPerfil; categoria?: CategoriaPerfil; }
  ...
  const onSubmit: SubmitHandler<FormValues> = async data => {
    ...
    if (data.imagenFile) {
      try {
        ...
        uploadedImageUrl = await uploadFileAndGetURL(data.imagenFile, filePath);
      } catch (err) {
    ...
    const provinciaNombre =
      currentUser.localidad?.provinciaNombre ||
      currentUser.localidad?.provincia?.nombre ||
      (typeof currentUser.provincia === 'string'
        ? currentUser.provincia
        : currentUser.provincia?.nombre) ||
      'Desconocida';
    ...
    try {
      ...
      const destino =
        originalRole === 'comercio'
          ? '/bienvenida?rol=comercio'
          : '/bienvenida?rol=prestador';
      router.push(destino);
    } catch (err) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <section>
            <h2 className="text-lg font-semibold text-texto-principal mb-2">
             ...
             {/* ... (nombrePublico, tituloCard, subtituloCard, descripcion sin cambios) ... */}
            <Controller
              name="nombrePublico"
              control={control}
              render={({ field }) => (
                <Input
                  id="nombrePublico"
                  label="Nombre Público del Comercio/Prestador*"
                  {...field}
                  value={field.value ?? ''}
                  placeholder="Ej: Mi Super Tienda / Juan Pérez Electricista"
                />
              )}
            />
            {errors.nombrePublico && (
              <p className="text-sm text-red-500 -mt-3 mb-3">
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  id="tituloCard"
                  label="Título para la Tarjeta (Opcional)"
                  {...field}
                  value={field.value ?? ''}
                  placeholder="Ej: ¡La Mejor Calidad! / Servicios 24hs"
                />
              )}
            />
            {errors.tituloCard && (
              <p className="text-sm text-red-500 -mt-3 mb-3">
                {errors.tituloCard.message}
              </p>
            )}
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  id="subtituloCard"
                  label="Subtítulo para la Tarjeta (Opcional)"
                  {...field}
                  value={field.value ?? ''}
                  placeholder="Ej: Expertos en tecnología / Tu solución en casa"
                />
              )}
            />
            {errors.subtituloCard && (
              <p className="text-sm text-red-500 -mt-3 mb-3">
                {errors.subtituloCard.message}
              </p>
            )}
            ...
            <Controller
              ...
              render={({ field }) => (
                <Textarea
                  ...
                  {...field}
                  value={field.value ?? ''}
                  placeholder="Describe tus servicios, productos, historia, etc."
                  rows={4}
                />
              )}
            />
          ...
          <section>
            ...
            <div className="grid md:grid-cols-2 gap-x-4 gap-y-0">
              ...
              />
              {errors.enlaceInstagram && (
                <p className="text-sm text-red-500 mt-1 mb-3">
                  {errors.enlaceInstagram.message}
                </p>
              )}
              ...
              />
              {errors.enlaceFacebook && (
                <p className="text-sm text-red-500 mt-1 mb-3">
                  {errors.enlaceFacebook.message}
                </p>
              )}
            </div>
          </section>
          ...
          <section>
            ...
            <Controller
              ...
              render={({ field }) => (
                <Input
                  id="direccionVisible"
                  label="Dirección Pública (Opcional, si quieres mostrarla)"
                  {...field}
                  value={field.value ?? ''}
            ...
            />
            {errors.direccionVisible && (
              <p className="text-sm text-red-500 -mt-3 mb-3">
                {errors.direccionVisible.message}
              </p>
            )}
            ...
            <div className="mt-2 space-y-1 text-sm text-texto-secundario bg-fondo-secundario p-3 rounded-md border border-borde-tarjeta">
              <p>
                <strong>Provincia:</strong> {provinciaDisplay}
              </p>
              <p>
                <strong>Localidad:</strong> {localidadDisplay}
              </p>
              {rolValido === 'comercio' && (
                <>
                  <p>
                    <strong>Rubro:</strong>{' '}
                    {currentUser.rubro?.rubro ?? 'No especificado'}
                  </p>
                  {currentUser.rubro?.subrubro && (
                    <p>
                      <strong>Sub-Rubro:</strong> {currentUser.rubro.subrubro}
                    </p>
                  )}
                </>
              )}
              {rolValido === 'prestador' && (
                <>
                  <p>
                    ...
                    {currentUser.categoria?.categoria ?? 'No especificada'}
                  </p>
                  {currentUser.categoria?.subcategoria && (
                    <p>
                      <strong>Sub-Categoría:</strong> {currentUser.categoria.subcategoria}
                    </p>
                  )}
                </>
              )}
              <p className="text-xs text-texto-placeholder mt-2">
                Esta información se toma de tu perfil y no es editable aquí.
              </p>
            </div>
          </section>
